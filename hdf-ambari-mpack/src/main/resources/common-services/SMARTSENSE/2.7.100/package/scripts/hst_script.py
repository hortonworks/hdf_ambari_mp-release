...
class HSTScript(Script):
    ...
    def start(self, env, upgrade_type=None):
        ...
        self.deploy_component_specific_config(env)
        if self.component == 'hst-server':
            self.start_hst_server()
        elif self.component == 'hst-agent':
    ...
    def unregister(self, env):
        if self.component == 'hst-agent':
            cmd = "{sudo} /usr/sbin/hst-agent unregister"
    ...
    def refresh_schedule(self, env):
        if self.component == 'hst-agent':
            cmd = "{sudo} /usr/sbin/hst-agent refresh-schedule"
            exit_code, output, error = self.execute_command(cmd)
    ...
    def capture(self, env):
        ...
        if self.component == 'hst-agent':
            ...
            for k, v in args.iteritems():
              if not v == None and v != 'None':
                extraArgs = '%s -C "%s=%s"' % (extraArgs, k, str(v))
            cmd = '{sudo} /usr/sbin/hst-agent capture %s %s %s -c "%s" %s &' % (diagnostics_services_cmd,
    ...
    def run_server_setup(self):
        cmd = " {sudo} /usr/sbin/hst-server setup -q --nostart --nocheck"
        print("Setting up HST server")
        exit_code, output, error = self.execute_command(cmd)


    def run_agent_setup(self):
        cmd = "{sudo} /usr/sbin/hst-agent setup -q &"
        try:
            exit_code, output, error = self.execute_command(cmd, background=True)
        except Exception, e:
    ...
    def install_packages(self):
        ...
        package_name = "smartsense-" + self.component

        if distname.startswith('ubuntu') or distname.startswith('debian'):
            cmd = "{sudo} dpkg-query -l  | grep  'ii\s*" + package_name + "' || {sudo} apt-get -o Dpkg::Options::=--force-confdef --allow-unauthenticated --assume-yes install " + package_name
        elif distname.startswith('sles') or distname.startswith('suse'):
            cmd = "{sudo} rpm -qa | grep " + package_name + " || {sudo} zypper install --auto-agree-with-licenses --no-confirm " + package_name
        else:
            cmd = "{sudo} rpm -qa | grep " + package_name + " || {sudo} yum -y install " + package_name
    ...
    def uninstall_hst_packages(self):
        ...
        distname, version = utils.get_os()
        removal_command = ""
        for package in params.smartsense_hst_packages:
            if distname.startswith('ubuntu') or distname.startswith('debian'):
    ...
    def upgrade_packages(self):
        ...
        if distname.startswith('ubuntu') or distname.startswith('debian'):
            cmd = "{sudo} apt-get -o Dpkg::Options::=--force-confdef --allow-unauthenticated --assume-yes install " + package_name
        elif distname.startswith('sles') or distname.startswith('suse'):
            cmd = "{sudo} zypper up --auto-agree-with-licenses --no-confirm " + package_name
        else:
            cmd = "{sudo} yum -y upgrade " + package_name
    ...
    def deploy_config(self, env, conf_file_name, conf_params):
        ...
        config.set('cluster', 'product.name', params.product_name)
        config.set('cluster', 'product.version', params.product_version)
        config.set('cluster', 'stack.version', params.stack_version)
        config.set('cluster', 'stack.root', params.stack_root)
        ...
        # # Write anonymization configuration
        anonymization_config = params.config['configurations'].get('anonymization-conf')
        if (anonymization_config != None):
            config = ConfigParser.RawConfigParser()
            if not 'security.anonymization.max.heap' in anonymization_config:
                anonymization_config['security.anonymization.max.heap'] = '2048'
            for k, v in anonymization_config.iteritems():
                key = k.split(".", 1)
                if (len(key) != 2):
                    sys.exit("Invalid property " + k + ". Configuration property should be named in the form <section name>.<property name>")
                if (str(v).lower() == "null") and config.has_option(key[0], key[1]):
            ...
            # First write to temp file and then rename to avoid partial written files.
            with open(params.anonymization_conf_file + ".tmp", 'w') as config_file:
                config.write(config_file)
            os.rename(params.anonymization_conf_file + ".tmp", params.anonymization_conf_file)
            cmd = "{sudo} chmod 644 " + params.anonymization_conf_file
            exit_code, output, error = self.execute_command(cmd, failOnError=True)
    ...
    def find_supported_services(self):
        import params
        cmd = "{sudo} /usr/sbin/hst-agent list-services -k"
        exit_code, output, error = self.execute_command(cmd, failOnError=False)
        if exit_code == 0:
            return set(re.findall(r"[\w']+", output.lower()))
        else:
            return ['accumulo', 'ambari', 'ams', 'atlas', 'cloudbreak', 'falcon', 'hbase', 'hdfs', 'hive', 'kafka', 'knox', 'logsearch', 'mr', 'oozie', 'pig', 'ranger', 'smartsense', 'spark2', 'spark', 'sqoop', 'storm', 'tez', 'yarn', 'zeppelin', 'zk']
    ...
    def am_i_namenode(self):
        ...
        if 'namenode_host' in params.config['clusterHostInfo']:
            ...
            if current_host in params.config['clusterHostInfo']['namenode_host']:
                ...
                return False
        elif 'namenode_hosts' in params.config['clusterHostInfo']:
            current_host = params.hostname
            print ("This host is identified as " + current_host + " and namenodes are identified as " + str(params.config['clusterHostInfo']['namenode_hosts']))
            if current_host in params.config['clusterHostInfo']['namenode_hosts']:
                return True
            else:
                return False
        else:
    ...
    def pre_upgrade_restart(self, env, upgrade_type=None):
        ...
        #  stack_select.select("smartsense", params.version)
    ...
    def validate_versions(self, is_this_retry=False):
        ...
        scripts_version = "${SmartSenseVersion}"
        rpm_version = None
        ...
        command = self.component + " --version"
        failure_message = "ERROR: Could not get " + self.component + " version. Please make sure smartsense-" + self.component + " package is installed on this host."
        ...
        message_suffix = "\n\nPerform following steps to finish the upgrade:\n" \
                         "1. Make sure that all the smartsense packages on all hosts are upgraded to the latest version ("+max_version+" or higher).\n" \
                         "2. run 'hst-agent upgrade-ambari-service' on Ambari server host to finish the upgrade.\n" \
        ...
        if (LooseVersion(rpm_version) < LooseVersion(scripts_version)):
            if is_this_retry == False:
                print(message_prefix + "SmartSense stack version is higher "
                      "(" + scripts_version + ") than installed smartsense-" + self.component + " package version (" + rpm_version + "). Upgrading smartsense-" +
                      self.component + " package on this host.")
                self.upgrade_packages()
                self.validate_versions(True)
            else:
                sys.exit(message_prefix + "SmartSense stack version is higher ("
                         + scripts_version + ") than installed smartsense-" + self.component + " package version (" + rpm_version + ")." + message_suffix)

        elif LooseVersion(rpm_version) < (LooseVersion(config_version)):
            if is_this_retry == False:
                print(message_prefix + "SmartSense configuration version in Ambari is higher ("
                      + config_version + ") than installed smartsense-" + self.component + " package version (" + rpm_version + "). Upgrading smartsense-"
                      + self.component + " package on this host.")
                self.upgrade_packages()
                self.validate_versions(True)
            else:
                sys.exit(message_prefix + "SmartSense configuration version in Ambari is higher ("
                         + config_version + ") than installed smartsense-" + self.component + " package version (" + rpm_version + ")." + message_suffix)

        elif (LooseVersion(config_version) < LooseVersion(rpm_version)):
            sys.exit(message_prefix + "SmartSense configuration version in Ambari is lower ("
                     + config_version + ") than installed smartsense-" + self.component + " package version (" + rpm_version + ")." + message_suffix)

        elif (LooseVersion(config_version) < LooseVersion(scripts_version)):
            sys.exit(message_prefix + "SmartSense configuration version in Ambari is lower ("
                     + config_version + ") than Ambari smartsense stack version (" + scripts_version + "). Please make sure that smartsense-" + self.component +
                     " package version on Ambari server host is latest (" + max_version + ") or higher." + message_suffix)

        elif (LooseVersion(scripts_version) < LooseVersion(rpm_version)):
            sys.exit(message_prefix + "SmartSense stack version is lower ("
                     + scripts_version + ") than installed smartsense-" + self.component + " package version (" + rpm_version + "). Please make sure that smartsense-"
                     + self.component + " package version on Ambari server host is latest (" + max_version +
                     ") or higher." + message_suffix)
        elif (LooseVersion(config_version) > LooseVersion(scripts_version)):
            sys.exit(message_prefix + "SmartSense configuration version in Ambari is higher ("
                     + config_version + ") Ambari smartsense  stack version (" + scripts_version + "). Please make sure that smartsense-" + self.component + " package version on Ambari server host is latest ("
                     + max_version + ") or higher." + message_suffix)
