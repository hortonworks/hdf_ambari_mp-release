...
ACTIVITY_ANALYZER_LIST = [ 'activity-analyzer' , 'hdfs-analyzer' , 'yarn-analyzer' , 'mapreduce-analyzer' , 'spark-analyzer', 'hive-analyzer']
...
class Activity(Script):
    ...
    def __init__(self, component = None):
        ...
        activity_type = self.component.split('-')[0].upper()
    ...
    def install(self, env):
        ...
        if self.component in ACTIVITY_ANALYZER_LIST:
            self.install_activity_analyzer()
        elif self.component == 'activity-explorer':
    ...
    def stop(self, env, upgrade_type = None):
        if self.component == 'activity-explorer':
            self.stop_activity_explorer()
        elif self.component in ACTIVITY_ANALYZER_LIST:
            self.stop_activity_analyzer()

    def status(self, env):
        if self.component == 'activity-explorer':
            self.activity_explorer_status(env)
        elif self.component in ACTIVITY_ANALYZER_LIST:
            self.activity_analyzer_status(env)


    def activity_analyzer_status(self, env):
        import params

        cmd = '/usr/sbin/activity-analyzer ' + activity_type + ' status '
    ...
    def activity_explorer_status(self, env):
        import params

        cmd = '/usr/sbin/activity-explorer status '
    ...
    def start_activity_analyzer(self):
        cmd = '/usr/sbin/activity-analyzer ' + activity_type + ' start '

        exit_code, output, error = self.execute_command(cmd)

    def stop_activity_analyzer(self):
        cmd = '/usr/sbin/activity-analyzer ' + activity_type + ' stop '
        exit_code, output, error = self.execute_command(cmd)

    def start_activity_explorer(self):
        import params
        cmd = 'export JAVA_HOME=' + params.java_home + '; /usr/sbin/activity-explorer start '
        exit_code, output, error = self.execute_command(cmd)

    def stop_activity_explorer(self):
        cmd = '/usr/sbin/activity-explorer stop '
        exit_code, output, error = self.execute_command(cmd)
    ...
    def install_packages(self):
        ...
        package_name = "smartsense-hst"
        if (self.component == "activity-explorer"):
            package_name = "smartsense-activity-explorer"
        elif (self.component.endswith("-analyzer")):
    ...
    def install_activity_explorer(self):
        import params
        import grp
        User(params.config['configurations']['activity-conf']['activity.explorer.user'],
             gid = params.config['configurations']['cluster-env']['user_group']
             )
        print ("Created explorer user  " + str(params.config['configurations']['activity-conf']['activity.explorer.user']))
    ...
    def get_analyzer_init_script_path(self):
        import params
        return os.path.join(params.activity_analyzer_home , self.component , "bin" , self.component)

    def get_explorer_init_script_path(self):
        import params
        return os.path.join(params.activity_explorer_home , "bin" , self.component)

    def get_os_specific_init_script_location(self, script_path):
    ...
    def install_init_script(self, script_path):

        init_script_location = self.get_os_specific_init_script_location(script_path)
        cmd = "{sudo} cp " + script_path + " " + init_script_location
        print("Setting up init script")
        exit_code, output, error = self.execute_command(cmd)

    def uninstall_init_script(self, script_path):

        init_script_location = self.get_os_specific_init_script_location(script_path)
        cmd = "{sudo} rm -f " + init_script_location + os.path.basename(script_path)
    ...
    def uninstall_activity_explorer(self):
        ...
        self.uninstall_init_script(self.get_explorer_init_script_path())

        distname, version = utils.get_os()
        cmd = "{sudo} rpm -e smartsense-activity-explorer"
        if distname.startswith('ubuntu') or distname.startswith('debian'):
            cmd = "{sudo} dpkg -r smartsense-activity-explorer ; dpkg --purge smartsense-activity-explorer "
        print("Removing smartsense-activity-explorer package")
        exit_code, output, error = self.execute_command(cmd, failOnError = False)

    def uninstall_activity_analyzer(self):
        import params

        self.uninstall_init_script(self.get_analyzer_init_script_path())

        # if this was the last analyzer on this host them remove rpm/deb package as well
        other_analyzer_exists = False
        if 'localComponents' in params.config:
            for component in params.config['localComponents']:
                if component.lower().endswith('-analyzer') and component.lower() != self.component:
                    other_analyzer_exists = True
                    break
        if (not other_analyzer_exists):
            distname, version = utils.get_os()
            cmd = "{sudo} rpm -e smartsense-activity-analyzer"
            if distname.startswith('ubuntu') or distname.startswith('debian'):
                cmd = "{sudo} dpkg -r smartsense-activity-analyzer ; dpkg --purge smartsense-activity-analyzer "
            print("Removing smartsense-activity-analyzer package")
            exit_code, output, error = self.execute_command(cmd, failOnError = False)
    ...
    def deploy_component_specific_config(self, env):
        ...
        if (params.is_ams_hbase_distributed == True and  self.am_i_metric_collector() == False):
            ...
            if ('configurations' in params.config and 'ams-hbase-site' in  params.config['configurations']):
                ...
                merged_ams_hbase_site['hbase.tmp.dir'] = '/var/lib/smartsense/activity-analysis/activity-analyzer/tmp/phoenix'
    ...
    def set_permissions(self, run_as_user = None):
        ...
        if self.component == 'activity-explorer':
            ...
            if 'activity-conf' in params.config['configurations'] and 'mapreduce.activity.analyzer.user.keytab' in params.config['configurations']['activity-conf']:
                exit_code, output, error = self.execute_command("{sudo} chown '" + user + "':'" + group + "' " + params.config['configurations']['activity-conf']['mapreduce.activity.analyzer.user.keytab'], failOnError = False)
            if 'activity-conf' in params.config['configurations'] and 'tez.activity.analyzer.user.keytab' in params.config['configurations']['activity-conf']:
                exit_code, output, error = self.execute_command("{sudo} chown '" + user + "':'" + group + "' " + params.config['configurations']['activity-conf']['tez.activity.analyzer.user.keytab'], failOnError = False)
            if 'activity-conf' in params.config['configurations'] and 'yarn.activity.analyzer.user.keytab' in params.config['configurations']['activity-conf']:
                exit_code, output, error = self.execute_command("{sudo} chown '" + user + "':'" + group + "' " + params.config['configurations']['activity-conf']['yarn.activity.analyzer.user.keytab'], failOnError = False)
