...
hst_log_dir = '/var/log/smartsense/hst'
if 'hst-log4j' in config['configurations'] and 'hst.log.dir' in config['configurations']['hst-log4j']:
        hst_log_dir = config['configurations']['hst-log4j']['hst.log.dir']

activity_log_dir = '/var/log/smartsense/activity-analysis'
if 'activity-log4j' in config['configurations'] and 'activity.log.dir' in config['configurations']['activity-log4j']:
        activity_log_dir = config['configurations']['activity-log4j']['activity.log.dir']

hst_conf_dir = '/etc/smartsense/hst/conf'
# Env properties
if 'hst-env' in config['configurations'] and 'hst.conf.dir' in config['configurations']['hst-env']:
        hst_conf_dir = config['configurations']['hst-env']['hst.conf.dir']

activity_conf_dir='/etc/smartsense/activity-analysis/conf'
if 'role' in config and config['role'].endswith('_ANALYZER'):
    activity_conf_dir = '/etc/smartsense/activity-analysis/activity-analyzer/conf'
elif 'role' in config and config['role'].endswith('_EXPLORER'):
    activity_conf_dir = '/etc/smartsense/activity-analysis/activity-explorer/conf'
if 'activity-env' in config['configurations'] and 'activity.conf.dir' in config['configurations']['activity-env']:
    activity_conf_dir = config['configurations']['activity-env']['activity.conf.dir']
...
hst_pid_dir = '/var/run/smartsense/hst'
if 'hst-env' in config['configurations'] and 'hst.pid.dir' in config['configurations']['hst-env']:
        hst_pid_dir = config['configurations']['hst-env']['hst.pid.dir']


activity_analyzer_pid_dir = '/var/run/smartsense/activity-analysis/activity-analyzer'
if 'activity-env' in config['configurations'] and 'activity.analyzer.pid.dir' in config['configurations']['activity-env']:
        activity_analyzer_pid_dir = config['configurations']['activity-env']['activity.analyzer.pid.dir']
activity_explorer_pid_dir = '/var/run/smartsense/activity-analysis/activity-explorer'
if 'activity-env' in config['configurations'] and 'activity.explorer.pid.dir' in config['configurations']['activity-env']:
        activity_analyzer_pid_dir = config['configurations']['activity-env']['activity.explorer.pid.dir']
...
smartsense_install_home = '/usr/lib/smartsense'
hst_install_home = os.path.join(smartsense_install_home, 'hst')
...
anonymization_conf_file = os.path.join(hst_conf_dir, 'anonymization.ini')
product_info_json = os.path.join(hst_conf_dir, 'productinfo.json')
smartsense_hst_packages = ["smartsense-hst-gateway", "smartsense-hst-agent", "smartsense-hst-server", "smartsense-hst"]

hst_view_dir = "/usr/lib/smartsense/ambari-service/SMARTSENSE/package/files/view"
ambari_view_dir = "/var/lib/ambari-server/resources/views"
hst_view_jar_file_name = 'smartsense-ambari-view-${SmartSenseVersion}.jar'
...
if 'hst-log4j' in config['configurations']:
    if 'hst.log.dir' in config['configurations']['hst-log4j']:
        hst_log_dir = config['configurations']['hst-log4j']['hst.log.dir']
    if 'hst.max.file.size' in config['configurations']['hst-log4j']:
        hst_max_file_size = config['configurations']['hst-log4j']['hst.max.file.size']
    if 'hst.max.backup.index' in config['configurations']['hst-log4j']:
        hst_max_backup_index = config['configurations']['hst-log4j']['hst.max.backup.index']
...
if 'activity-log4j' in config['configurations']:
    if 'activity.log.dir' in config['configurations']['activity-log4j']:
        activity_log_dir = config['configurations']['activity-log4j']['activity.log.dir']
    if 'activity.max.file.size' in config['configurations']['activity-log4j']:
        activity_max_file_size = config['configurations']['activity-log4j']['activity.max.file.size']
    if 'activity.max.backup.index' in config['configurations']['activity-log4j']:
        activity_max_backup_index = config['configurations']['activity-log4j']['activity.max.backup.index']


activity_analysis_home = os.path.join(smartsense_install_home, "activity-analysis")
activity_explorer_home = os.path.join(activity_analysis_home, "activity-explorer")
activity_analyzer_home = os.path.join(activity_analysis_home, "activity-analyzer")
activity_explorer_notebook_dir = "/var/lib/smartsense/activity-analysis/activity-explorer/notebook"

hst_server_owned_dirs = [
                    '/var/run/smartsense:755',
                    '/var/lib/smartsense:755',
                    '/var/log/smartsense:755',
                    '/etc/smartsense:755',
                    '/etc/smartsense/hst',
                    hst_log_dir+':750',
                    hst_conf_dir,
                    hst_pid_dir,
                    "/var/lib/smartsense/hst/hst-server"
                    ]
activity_analyzer_owned_dirs = [
                    ...
                    '/var/log/smartsense:755',
                    '/etc/smartsense:755',
                    '/var/lib/smartsense/activity-analysis/activity-analyzer',
                    '/etc/smartsense/activity-analysis/activity-analyzer',
                    '/var/log/smartsense/activity-analysis/activity-analyzer:750',
                    '/var/run/smartsense/activity-analysis/activity-analyzer',
...
activity_explorer_owned_dirs = [
                    ...
                    '/var/log/smartsense:755',
                    '/etc/smartsense:755',
                    '/var/lib/smartsense/activity-analysis/activity-explorer',
                    '/etc/smartsense/activity-analysis/activity-explorer',
                    '/var/log/smartsense/activity-analysis/activity-explorer:750',
                    '/var/run/smartsense/activity-analysis/activity-explorer',
                    activity_log_dir+':750',
                    activity_conf_dir,
...
hst_agent_owned_dirs = [
                    ...
                    hst_log_dir+':750',
                    hst_conf_dir,
                    '/var/lib/smartsense/hst/hst-agent'
                    ]
...
if ('hst-agent-conf' in config['configurations'] and 'agent.tmp.dir' in config['configurations']['hst-agent-conf']) :
    hst_agent_owned_dirs.append(config['configurations']['hst-agent-conf']['agent.tmp.dir'])
...
# we do not want these keytab directories to be created
#activity_analyzer_owned_dirs.append(activity_analyzer_keytab)
#activity_explorer_owned_dirs.append(activity_explorer_keytab)
...
if 'activity-env' in config['configurations'] and 'analyzer.jvm.heap' in config['configurations']['activity-env']:
    analyzer_jvm_heap = config['configurations']['activity-env']['analyzer.jvm.heap']
if 'activity-env' in config['configurations'] and 'analyzer.jvm.opts' in config['configurations']['activity-env']:
    analyzer_jvm_opts = config['configurations']['activity-env']['analyzer.jvm.opts']
