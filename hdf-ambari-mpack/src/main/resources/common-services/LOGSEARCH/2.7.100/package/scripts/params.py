...
def is_scheme_hdfs_or_file(value):
  if value and ("hdfs" in value.lower() or "file" in value.lower()):
    return True
  return False

def compute_hdfs_path(value):
  if value and is_scheme(value):
    splitted = value.split('://')
    if is_scheme_hdfs_or_file(splitted[0]):
...
logsearch_properties['logsearch.solr.metadata.collection'] = 'logsearch_metadata'
logsearch_properties['logsearch.solr.metadata.config.name'] = 'logsearch_metadata'
logsearch_properties['logsearch.solr.metadata.replication.factor'] = '2'
...
if logsearch_properties['logsearch.solr.audit.logs.use.ranger'] == 'false':
  del logsearch_properties['logsearch.solr.ranger.audit.logs.collection.name']
...
logsearch_solr_collection_service_logs = logsearch_properties['logsearch.solr.service.logs.collection']
logsearch_collection_service_logs_numshards = logsearch_properties['logsearch.solr.service.logs.numshards']

logsearch_solr_collection_audit_logs = logsearch_properties['logsearch.solr.audit.logs.collection']
logsearch_collection_audit_logs_numshards = logsearch_properties['logsearch.solr.audit.logs.numshards']
...
logfeeder_properties['logfeeder.solr.metadata.collection'] = 'logsearch_metadata'
...
cloud_storage_mode = logfeeder_properties['logfeeder.cloud.storage.mode']
logfeeder_checkpoint_folder = logfeeder_properties['logfeeder.checkpoint.folder']
logfeeder_rollover_archive_dir = logfeeder_properties['logfeeder.cloud.rollover.archive.base.dir']
logfeeder_cloud_storage_base_path = logfeeder_properties['logfeeder.cloud.storage.base.path']

logfeeder_storage_base_path = compute_hdfs_path(logfeeder_cloud_storage_base_path)

cloud_supported = True if logfeeder_properties['logfeeder.cloud.storage.mode'] == 'cloud' or logfeeder_properties['logfeeder.cloud.storage.mode'] == 'hybrid' else False
create_hdfs_folder_on_startup = True if default_fs and not default_fs.startswith("file://") and cloud_supported else False

logfeeder_use_solr_cloud_client = default("configurations/logfeeder-env/logfeeder_use_solr_cloud_client", True)
if not logfeeder_use_solr_cloud_client:
  if logsearch_use_external_solr:
    logfeeder_properties['logfeeder.solr.cloud.client.discover'] = 'true'
  else:
   infra_solr_hosts = default("/clusterHostInfo/infra_solr_hosts", [])
   if len(infra_solr_hosts) > 0:
     infra_solr_host_addresses = []
     for infra_solr_host in infra_solr_hosts:
       infra_solr_ssl_enabled = default('configurations/infra-solr-env/infra_solr_ssl_enabled', False)
       ...
       infra_solr_host_addresses.append(format("{infra_solr_protocol}://{infra_solr_host}:{infra_solr_port}/solr"))
     logfeeder_properties['logfeeder.solr.urls']=",".join(infra_solr_host_addresses)
     logfeeder_properties['logfeeder.solr.cloud.client.discover'] = 'false'

logfeeder_custom_fs = default("configurations/logfeeder-properties/logfeeder.cloud.storage.custom.fs", None)
is_hdfs_filesystem = False

if not security_enabled:
  if is_scheme_hdfs_or_file(logfeeder_custom_fs):
    is_hdfs_filesystem = True
  elif is_scheme_hdfs_or_file(default_fs):
    is_hdfs_filesystem = True

if is_hdfs_filesystem:
  logfeeder_hdfs_user = format(default("configurations/logfeeder-properties/logfeeder.hdfs.user", logsearch_user))
  logfeeder_properties['logfeeder.hdfs.user'] = logfeeder_hdfs_user
else:
  logfeeder_properties['logfeeder.hdfs.user'] = ''
...
#####################################
# Log Search Config API settings
#####################################

logsearh_zk_config_api_shared_inputs = default('configurations/logsearch-common-env/logsearch_shared_zk_configs', True)
logsearh_config_api_filter_storage = default('configurations/logsearch-common-env/logsearch_log_level_filter_storage', "solr")

if not logsearh_zk_config_api_shared_inputs:
  logsearch_properties['logsearch.config.api.enabled'] = 'false'
  logfeeder_properties['logfeeder.configs.local.enabled'] = 'true'
  solr_filter_storage = "true" if logsearh_config_api_filter_storage == "solr" else "false"
  zookeeper_filter_storage = "true" if logsearh_config_api_filter_storage == "zookeeper" else "false"
  logsearch_properties['logsearch.config.api.filter.solr.enabled'] = solr_filter_storage
  logfeeder_properties['logfeeder.configs.filter.solr.enabled'] = solr_filter_storage
  logsearch_properties['logsearch.config.api.filter.zk.enabled'] = zookeeper_filter_storage
  logfeeder_properties['logfeeder.configs.filter.zk.enabled'] = zookeeper_filter_storage
...
smoke_logsearch_cmd = format("curl -k -sS -w '%{{http_code}}' {logsearch_protocol}://{logsearch_server_host}:{logsearch_ui_port}/api/v1/info | sed '/200/!{{$q1}}'")

#######################################
# Logsearch Server limits conf template
#######################################
limits_conf_dir = "/etc/security/limits.d"
logsearch_user_nofile_limit = default("/configurations/logsearch-env/logsearch_user_nofile_limit", "1024")
logsearch_user_nproc_limit = default("/configurations/logsearch-env/logsearch_user_nproc_limit", "1024")

#######################################
# HDFS resource configurations
#######################################
if default_fs:
