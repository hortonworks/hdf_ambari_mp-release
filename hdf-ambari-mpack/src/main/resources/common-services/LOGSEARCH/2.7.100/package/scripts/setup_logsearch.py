...
def setup_logsearch():
  ...
  File(format("{logsearch_server_conf}/log4j2.yml"),
  ...
  if params.cloud_storage_mode != 'cloud':
    if not params.logsearch_use_external_solr:
      check_znode()
  ...
  File(os.path.join(params.limits_conf_dir, 'logsearch.conf'),
       owner='root',
       group='root',
       mode=0644,
       content=Template("logsearch.conf.j2")
       )
