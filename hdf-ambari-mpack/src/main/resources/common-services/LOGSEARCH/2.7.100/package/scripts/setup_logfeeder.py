...
def setup_logfeeder():
  ...
  Directory([params.logfeeder_log_dir, params.logfeeder_pid_dir, params.logfeeder_checkpoint_folder, params.logfeeder_rollover_archive_dir],
            mode=0755,
            cd_access='a',
            create_parents=True
            )
  ...
  if params.default_fs:
    XmlConfig("core-site.xml",
              conf_dir=params.logsearch_logfeeder_conf,
    ...
    if params.create_hdfs_folder_on_startup \
      and (not params.is_scheme(params.logfeeder_cloud_storage_base_path) or params.is_scheme_hdfs_or_file(params.logfeeder_cloud_storage_base_path)):
      try:
        params.HdfsResource(params.logfeeder_storage_base_path,
                          type="directory",
        ...
        Logger.warning("Creating logsearch app HDFS working directory failed with error:\n{0}".format(err))
  ...
  File(format("{logsearch_logfeeder_conf}/log4j2.yml"),
